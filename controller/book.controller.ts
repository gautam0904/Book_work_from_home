...
export const getbook = async (req: Request, res: Response) => {
    const { page, pagesize, searchedBook, author,category} = req.query;
    try {
        if (!page && !pagesize) {
            const ret = await book.getbook((page as string), (pagesize as string), (searchedBook as string),(author as string),(category as string));
            return res.status((ret as { status: number, content: object }).status).json({
                "response": (ret as { status: number, content: object }).content
            })
        } else {
